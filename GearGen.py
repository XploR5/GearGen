...
class Involute:
    ...
    def draw(self, sketch, zShift=0, rotation=0, involutePointCount=10):
        ...
        radiusStep = (self.gear.outsideDiameter / 2 -
                      involuteFromRad) / (involutePointCount - 1)
        involuteIntersectionRadius = involuteFromRad
        for i in range(0, involutePointCount):
            newPoint = self.InvolutePoint(
                self.gear.baseDiameter / 2.0, involuteIntersectionRadius, zShift)
        ...
        for i in range(0, involutePointCount):
            involute2Points.append(adsk.core.Point3D.create(
                involutePoints[i].x, -involutePoints[i].y, zShift))
        ...
        assert len(crossPoints) == 0 or len(
            crossPoints) == 1, 'Failed to compute a valid involute profile!'
        if len(crossPoints) == 1:
            ...
            if self.gear.toothCount >= 100:
                ...
                tipCurve1Angle = math.atan2(
                    involutePoints[-1].y, involutePoints[-1].x)
                tipCurve2Angle = math.atan2(
                    involute2Points[-1].y, involute2Points[-1].x)
        ...
        rootArc = sketch.sketchCurves.sketchArcs.addByCenterStartSweep(
            ...
            adsk.core.Point3D.create(math.cos(curve1Angle) * (self.gear.rootDiameter / 2 - 0.01),
                                     math.sin(curve1Angle) *
                                     (self.gear.rootDiameter / 2 - 0.01),
                                     zShift),
        ...
        if True:
            ...
            rootArc.deleteMe()
            sketch.sketchCurves.sketchLines.addByTwoPoints(
                originPoint, spline1.startSketchPoint)
            sketch.sketchCurves.sketchLines.addByTwoPoints(
                originPoint, spline2.startSketchPoint)
...
class HelicalGear:
    ...
    def createInNormalSystem(toothCount, normalModule, normalPressureAngle, helixAngle, backlash=0, addendum=1,
        ...
        gear.pressureAngle = math.atan2(
            math.tan(gear.normalPressureAngle), cosHelixAngle)
    ...
    def createInRadialSystem(toothCount, radialModule, radialPressureAngle, helixAngle, backlash=0, addendum=1,
        ...
        radialPressureAngle = radialPressureAngle if 0 <= radialPressureAngle < math.radians(
            90) else 0
        helixAngle = helixAngle if math.radians(
            -90) < helixAngle < math.radians(90) else 0
        ...
        gear.normalPressureAngle = math.atan(
            math.tan(radialPressureAngle) * math.cos(gear.helixAngle))
        gear.normalCircularPitch = gear.normalModule * math.pi
    ...
    def modelGear(self, parentComponent, sameAsLast=False):
        ...
        occurrence = parentComponent.occurrences.addNewComponent(
        ...
        if (not (sameAsLast and lastGear)):
            ...
            if (not self.herringbone):
                ...
                for i in range(self.toothCount):
                    involute.draw(
                        sketch, 0, (i / self.toothCount) * 2 * math.pi)
                # Base Circle
            ...
            if (not self.herringbone):
                ...
                sweepInput.twistAngle = adsk.core.ValueInput.createByReal(
                    -self.tFor(self.width))
                if (baseFeature):
                    sweepInput.targetBaseFeature = baseFeature
                gearBody = sweepFeature = component.features.sweepFeatures.add(
                    sweepInput).bodies.item(0)
            else:
                path1 = component.features.createPath(line1)
                sweepInput = component.features.sweepFeatures.createInput(profs, path1,
                                                                          adsk.fusion.FeatureOperations.NewBodyFeatureOperation)
                sweepInput.twistAngle = adsk.core.ValueInput.createByReal(
                    -self.tFor(self.width / 2))
                if (baseFeature):
                    sweepInput.targetBaseFeature = baseFeature
        ...
        pitchDiameterSketch = component.sketches.add(
            component.xYConstructionPlane)
        pitchDiameterSketch.name = "PD: {0:.3f}mm".format(
            self.pitchDiameter * 10)
...
class RackGear:
    ...
    def rackLines(self, x, y, z, m, n, height, pAngle, hAngle, backlash, addendum, dedendum):
        ...
        addendum = min(addendum, (-(1 / 4) * (backlash - P)
                       * (1 / math.tan(pAngle))) - 0.0001)
        dedendum = min(dedendum, -(1 / 4) * (-backlash - P)
                       * (1 / math.tan(pAngle)) - 0.0001)
        ...
        for i in range(n):
            ...
            lines.append(
                adsk.core.Line3D.create(adsk.core.Point3D.create(
                    x + ((i * P) + (P / 2) + backlash / 2 -
                         (math.tan(pAngle) * 2 * dedendum)) * strech, y,
                    z - dedendum),
                    adsk.core.Point3D.create(x + ((i * P) + (P / 2) + backlash / 2 - (
            ...
            lines.append(
                adsk.core.Line3D.create(adsk.core.Point3D.create(
                    x + ((i * P) + P - (math.tan(pAngle) *
                         (dedendum + addendum))) * strech, y,
                    z + addendum),
        ...
        if (not (sameAsLast and lastGear)):
            ...
            if (self.herringbone):
                wireBody1, _ = tbm.createWireFromCurves(self.rackLines(
                    -self.length / 2 -
                    (math.tan(abs(self.helixAngle)) +
                     math.tan(self.helixAngle)) * self.width / 2,
                    -self.width / 2,
                    0,
                ...
                wireBody2, _ = tbm.createWireFromCurves(self.rackLines(
                    -self.length / 2 -
                    math.tan(abs(self.helixAngle)) * self.width / 2,
                    0,
                    0,
                    self.normalModule, teeth, self.height, self.normalPressureAngle, self.helixAngle,
                ...
                wireBody1, _ = tbm.createWireFromCurves(self.rackLines(
                    -self.length / 2 -
                    (math.tan(abs(self.helixAngle)) +
                     math.tan(self.helixAngle)) * self.width,
                    -self.width / 2,
                    0,
                    self.normalModule, teeth, self.height, self.normalPressureAngle, self.helixAngle,
                    self.backlash, self.addendum, self.dedendum
                ...
                wireBody2, _ = tbm.createWireFromCurves(self.rackLines(
                    -self.length / 2 -
                    math.tan(abs(self.helixAngle)) * self.width,
                    self.width / 2,
                    0,
                    self.normalModule, teeth, self.height, self.normalPressureAngle, self.helixAngle,
                    self.backlash, self.addendum,
            ...
            if (self.herringbone):
                tempBRepBodies.append(
                    tbm.createFaceFromPlanarWires([wireBody3]))
            else:
                tempBRepBodies.append(
                    tbm.createFaceFromPlanarWires([wireBody2]))
            # Creates the ruled surface connectiong the two end caps
            tempBRepBodies.append(tbm.createRuledSurface(
                wireBody1.wires.item(0), wireBody2.wires.item(0)))
            if (self.herringbone):
                tempBRepBodies.append(tbm.createRuledSurface(
                    wireBody2.wires.item(0), wireBody3.wires.item(0)))
            ...
            body = component.features.boundaryFillFeatures.add(
                boundaryFillInput).bodies.item(0)
...
class CommandCreatedHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            # tabLLM = inputs.addTabCommandInput("TabProperties", "LLM")

            # # We Take User Inputs Here
            # # This is to input and output to the LLM
            # # Add value input for llm_question
            # llm_question = tabLLM.children.addValueInput("llm_question", "LLM Question", "deg",
            #                                              adsk.core.ValueInput.createByReal(pers['VIHelixAngle']))
            # llm_question.tooltip = "LLM question"
            # llm_question.tooltipDescription = "Enter your question here"

            # # Add value input for llm_answer
            # llm_answer = tabLLM.children.addValueInput("llm_answer", "LLM Answer", "deg",
            #                                            adsk.core.ValueInput.createByReal(pers['VIHelixAngle']))
            # llm_answer.tooltip = "LLM answer"
            # llm_answer.tooltipDescription = "Enter your answer here"

            # This is to select the Type
            ddType = tabSettings.children.addDropDownCommandInput(
                "DDType", "Type", 0)
            # on click this redirects to resource
            ddType.listItems.add(
                "External Gear", pers['DDType'] == "External Gear", "resources/external")
            ddType.listItems.add(
                "Internal Gear", pers['DDType'] == "Internal Gear", "resources/internal")
            ddType.listItems.add(
                "Rack Gear", pers['DDType'] == "Rack Gear", "resources/rack")

            # This is to select the Module
            # Setting command Inputs # This is a drop down
            ...
            # This is to select the Helix Angle
            ...
            # This is to select the number of Teeth
            ...
            # This is to select the Gear Width
            ...
            # This is for Internal Gear Outside Diameter
            ...
            # This is to make the gear Herringbone
            ...
            # This option is to enable the live preview of the gear, it is not realtime as of now
            # This may slow system down
            bvPreview = tabSettings.children.addBoolValueInput(
                "BVPreview", "Preview", True, "", pers['BVPreview'])
            ...
            ddStandard = tabAdvanced.children.addDropDownCommandInput(
                "DDStandard", "Standard", 0)
            ddStandard.listItems.add(
                "Normal", pers['DDStandard'] == "Normal", "resources/normal")
            ddStandard.listItems.add(
                "Radial", pers['DDStandard'] == "Radial", "resources/radial")
            ...
            tbWarning2 = tabAdvanced.children.addTextBoxCommandInput(
                "TBWarning2", "", '', 2, True)

            # Position
            siPlane = tabPosition.children.addSelectionInput(
                "SIPlane", "Plane", "Select Gear Plane")
            ...
            siDirection = tabPosition.children.addSelectionInput(
                "SIDirection", "Line", "Select Rack Direction")
            ...
            siOrigin = tabPosition.children.addSelectionInput(
                "SIOrigin", "Center", "Select Gear Center")
            ...
            bvFlipped = tabPosition.children.addBoolValueInput(
                "BVFlipped", "Flip", True)
            bvFlipped.isVisible = False
            bvFlipped.tooltip = "Flips rack direction"

            ddDirection = tabPosition.children.addDropDownCommandInput(
                "DDDirection", "Direction", 0)
            ...
            # Properties
            tbProperties = tabProperties.children.addTextBoxCommandInput(
                "TBProperties", "", "", 5, True)
...
class CommandInputChangedHandler(adsk.core.InputChangedEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            # This Handles input visibillity based on gear type
            if (args.input.id == "DDType"):
                gearType = args.input.selectedItem.name
                args.inputs.itemById(
                    "ISTeeth").isVisible = gearType != "Rack Gear"
                args.inputs.itemById(
                    "VIHeight").isVisible = gearType == "Rack Gear"
                args.inputs.itemById(
                    "VILength").isVisible = gearType == "Rack Gear"
                args.inputs.itemById(
                    "VIDiameter").isVisible = gearType == "Internal Gear"
            # Updates Information
            ...
            if (not args.input.id[:2] == "TB"):
                isInvalid = generateGear(
                    args.input.parentCommand.commandInputs).isInvalid
                if (isInvalid):
                    ...
                    args.input.parentCommand.commandInputs.itemById(
                        "TBWarning1").formattedText = ''
                    args.input.parentCommand.commandInputs.itemById(
                        "TBWarning2").formattedText = ''
            ...
            if (args.input.id == "APITabBar"):
                if (args.inputs.itemById("TabPosition") and args.inputs.itemById("TabPosition").isActive):
                    args.input.parentCommand.commandInputs.itemById(
                        "SIOrigin").isVisible = True
                    args.input.parentCommand.commandInputs.itemById(
                        "SIPlane").isVisible = True
                    args.input.parentCommand.commandInputs.itemById(
                        "DVOffsetZ").isVisible = True
                    if (args.input.parentCommand.commandInputs.itemById("DDType").selectedItem.name == "Rack Gear"):
                        args.input.parentCommand.commandInputs.itemById(
                            "SIDirection").isVisible = True
                        args.input.parentCommand.commandInputs.itemById(
                            "DVOffsetX").isVisible = True
                        args.input.parentCommand.commandInputs.itemById(
                            "DVOffsetY").isVisible = True
                        args.input.parentCommand.commandInputs.itemById(
                            "BVFlipped").isVisible = True
                    else:
                        args.input.parentCommand.commandInputs.itemById(
                            "AVRotation").isVisible = True
                else:
                    args.input.parentCommand.commandInputs.itemById(
                        "SIOrigin").isVisible = False
                    args.input.parentCommand.commandInputs.itemById(
                        "SIDirection").isVisible = False
                    args.input.parentCommand.commandInputs.itemById(
                        "SIPlane").isVisible = False
                    args.input.parentCommand.commandInputs.itemById(
                        "DVOffsetX").isVisible = False
                    args.input.parentCommand.commandInputs.itemById(
                        "DVOffsetY").isVisible = False
            ...
            if (args.input.id in ["SIOrigin", "SIDirection", "SIPlane", "AVRotation", "DVOffsetX", "DVOffsetY",
                ...
                if (args.input.parentCommand.commandInputs.itemById("DDType").selectedItem.name != "Rack Gear"):
                    mat = regularMoveMatrix(
                        args.input.parentCommand.commandInputs)
                    ...
                    d0 = d.copy()
                    d0.normalize()
                    d0.scaleBy(args.input.parentCommand.commandInputs.itemById(
                        "DVOffsetZ").value)
                    p0 = p.copy()
                    p0.subtract(d0)
                    ...
                    args.input.parentCommand.commandInputs.itemById(
                        "DVOffsetZ").setManipulator(p0.asPoint(), d)
                    ...
                    x0 = xf.copy()
                    x0.normalize()
                    x0.scaleBy(args.input.parentCommand.commandInputs.itemById(
                        "DVOffsetX").value)

                    y0 = y.copy()
                    y0.normalize()
                    y0.scaleBy(args.input.parentCommand.commandInputs.itemById(
                        "DVOffsetY").value)

                    z0 = z.copy()
                    z0.normalize()
                    z0.scaleBy(args.input.parentCommand.commandInputs.itemById(
                        "DVOffsetZ").value)
                    ...
                    args.input.parentCommand.commandInputs.itemById(
                        "DVOffsetX").setManipulator(px.asPoint(), xf)
                    args.input.parentCommand.commandInputs.itemById(
                        "DVOffsetY").setManipulator(py.asPoint(), y)
                    args.input.parentCommand.commandInputs.itemById(
                        "DVOffsetZ").setManipulator(pz.asPoint(), z)
...
def moveMatrixPdro(position, direction, rotation, offset):
    ...
    mat.setToAlignCoordinateSystems(
        ...
        adsk.core.Vector3D.create(-math.sin(-rotation),
                                  math.cos(-rotation), 0),
        adsk.core.Vector3D.create(0, 0, 1),
        position,
        p.uDirection,
...
def moveMatrixPxzfxyz(position, x, z, flip, offsetX, offsetY, offsetZ):
    ...
    p = adsk.core.Plane.createUsingDirections(
        adsk.core.Point3D.create(0, 0, 0), z, x)
...
def stop(context):
    try:
        ...
        for panel in TOOLBARPANELS:
            p = ui.allToolbarPanels.itemById(
                panel).controls.itemById(COMMANDID)
            if p:
                p.deleteMe()
